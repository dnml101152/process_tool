...
class MainView(QMainWindow):
    ...
    def save_book_note(self):
        """
        Saves the current note to the database after extracting all relevant metadata.
        Handles missing data and structure gracefully.
        """

        # Check if current_note exists
        if not hasattr(self, "current_note") or self.current_note is None:
            print("[ERROR] No current note set. Aborting save.")
            return

        try:
            # Safely get selected tag IDs
            self.current_note.book_tags_id = self.tag_widget.get_selected_tag_ids() or []

            # Create a lookup dict from tags list to map id -> title
            id_to_title = {
                doc["id"]: doc["title"]
            ...
            # Resolve tag titles from IDs, fallback to "Unknown ID" if not found
            self.current_note.book_tags = [
                id_to_title.get(tag_id, "Unknown ID")
                for tag_id in self.current_note.book_tags_id
            ]

            # Retrieve the selected book_id from combo box
            self.current_note.book_id = self.source_combo.currentData()

            # Attempt to find book document by book_id
            book_doc = self.books_collection.find_one({"book_id": self.current_note.book_id})

            if book_doc:
                self.current_note.book_title = book_doc.get("title", "Unknown Title")
                last = book_doc.get("author", "Doe")
                first = book_doc.get("first_name", "John")
                self.current_note.book_author = f"{last}, {first}"
            else:
                # Fallback if no book is found
                self.current_note.book_title = "Unknown Title"
                self.current_note.book_author = "Doe, John"

            # Get source and comment text from UI
            ...
            # Get page number from spin widget
            ...
            self.notes_col.insert_one(note_doc)

            # Clear tag selection (UX improvement)
            self.tag_widget.clear_tags()
