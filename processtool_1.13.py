...
from PyQt5.QtWidgets import (
    ...
    QVBoxLayout, QHBoxLayout, QListWidgetItem, QLineEdit, QDialog, QComboBox, QCheckBox, QScrollArea, QMenu, QAction, QColorDialog,QTextEdit
)
...
class RunProcessWidget(QWidget):
    def __init__(self, main):
        super().__init__()
        self.main = main  # Reference to main app controller
    ...
    def update_view(self):
        """
        Update the visible step widget and labels according to current_index.
        This version handles missing or malformed data safely.
        """
        if (
            not self.process_data or
            not isinstance(self.process_data, dict) or
            "sub_process_data" not in self.process_data or
            not isinstance(self.process_data["sub_process_data"], list) or
            self.current_index >= len(self.process_data["sub_process_data"]) or
            self.current_index < 0
        ):
            # Invalid data or index out of range; do nothing safely
            return

        current_step = self.process_data["sub_process_data"][self.current_index]
        if not isinstance(current_step, dict):
            # Current step is malformed, skip update
            return
        ...
        total_steps = len(self.process_data["sub_process_data"])
        self.sub_label.setText(f"Step {self.current_index + 1} of {total_steps}")

        widget = self.widgets_by_type.get(step_type)
        if not widget:
            # No widget available for this step type; skip update
            return

        # Safely call widget methods if they exist
        try:
            ...
            # Log error or handle gracefully, here just print
            print(f"Error updating widget for step type '{step_type}': {e}")
            return
    ...
    def go_next_or_finish(self):
        """
        Validate input of current step, save data, and move forward or finish.
        Includes safety checks and error handling.
        """
        # Validate that process data and current index are valid
        if (
            not self.process_data or
            "sub_process_data" not in self.process_data or
            not (0 <= self.current_index < len(self.process_data["sub_process_data"]))
        ):
            print("Process data invalid or index out of range.")
            return

        current_step = self.process_data["sub_process_data"][self.current_index]
        if not isinstance(current_step, dict):
            print("Current step data malformed.")
            return

        step_type = current_step.get("type")
        widget = self.widgets_by_type.get(step_type)

        # Validate widget existence and input validity
        if not widget or not callable(getattr(widget, "is_valid", None)) or not widget.is_valid():
            print("Invalid input for this step.")
            return

        # Initialize tracking dict if None
        if self.process_tracking is None:
            self.process_tracking = {"responses": []}

        # Attempt to get data from widget safely
        try:
            data = widget.get_data() if callable(getattr(widget, "get_data", None)) else None
        except Exception as e:
            print(f"Error getting data from widget: {e}")
            return

        # Append response safely
        self.process_tracking.setdefault("responses", []).append({
            "step": self.current_index,
            "type": step_type if step_type else "",
            "data": data
        })

        self.current_index += 1

        if self.current_index < len(self.process_data.get("sub_process_data", [])):
            self.update_view()
        else:
            # Process finished; mark status and save to DB with error handling
            self.process_tracking["status"] = "Completed"
            self.process_tracking["end_time"] = datetime.now()

            try:
                if hasattr(self.main, "answer_collection") and callable(getattr(self.main.answer_collection, "insert_one", None)):
                    self.main.answer_collection.insert_one(self.process_tracking)
            except Exception as e:
                print(f"Error saving process tracking data: {e}")

            # Switch to main grid widget safely
            try:
    ...
    def cancel_process(self):
        """
        Cancel the running process and save cancellation status.
        """
        ...
        # Reset process data and switch UI
        self.process_tracking = None
        self.process_id = None
        ...
        try:
            ...
            print(f"Error switching to grid widget after cancellation: {e}")

    def start_process(self, process_data):
        """
        Initialize a new process run.
        """
        # Defensive checks on input data
        if not process_data or not isinstance(process_data, dict):
            print("Invalid process data passed to start_process.")
            return
...
class AddSubDialog(QDialog):
    def __init__(self, parent=None, edit=None):
        ...
        self.edit = edit  # Store the dict passed in for pre-filling inputs
        ...
        self.ok_button.setEnabled(False)  # Disable OK until input is valid

        # --- Safely set combo box index based on edit type ---
        if edit:
            type_str = edit.get("type", "")
            # Find index of the type string safely; fallback to 0 if not found
            index = self.type_combo.findText(type_str, Qt.MatchFixedString)
            if index < 0:
                index = 0  # Safe fallback to first type option
                # Could log warning here if needed about unknown type_str
            self.type_combo.setCurrentIndex(index)
        else:
            self.type_combo.setCurrentIndex(0)

        # Connect signal AFTER setting index to avoid premature triggers
        self.type_combo.currentIndexChanged.connect(self.rebuild_entry)

        # Build input fields for the current selected type
        self.rebuild_entry()

        # --- Defer prefill to ensure widgets exist after rebuild_entry ---
        if edit:
            QTimer.singleShot(0, self.prefill_edit)

    def prefill_edit(self):
        # Prefill input widgets based on edit dict data safely
        subtype = self.type_combo.currentText()
        if subtype == "multicheck":
            # Safely get title and items with defaults and type checks
            title = self.edit.get("text", "")
            items = self.edit.get("items", [])
            if not isinstance(items, list):
                items = []  # Fallback to empty list if not proper list

            if self.text_title:
                self.text_title.setText(title)  # Set checklist title
            if self.text_widget:
                self.text_widget.setPlainText("\n".join(items))  # Set multiline checklist items
        else:
            # For other subtypes, expect single-line text input
            text = self.edit.get("text", "")
            if self.text_widget and isinstance(self.text_widget, QLineEdit) and isinstance(text, str):
                self.text_widget.setText(text)  # Set the QLineEdit text safely

    def validate_input(self):
        # Validate current inputs to enable/disable OK button accordingly
        subtype = self.type_combo.currentText()

        if subtype == "multicheck":
            valid = False
            # Ensure text_title and text_widget exist before accessing their text
            if self.text_title is not None and self.text_widget is not None:
                valid = bool(
                    self.text_title.text().strip() and
                    self.text_widget.toPlainText().strip()
                )
        else:
            valid = False
            # For other types, ensure text_widget exists and is QLineEdit
            if self.text_widget is not None and isinstance(self.text_widget, QLineEdit):
                valid = bool(self.text_widget.text().strip())

        self.ok_button.setEnabled(valid)  # Enable OK only if input is valid

    def get_data(self):
        # Return current data in a dict format, safe access with fallbacks
        subtype = self.type_combo.currentText()

        if subtype == "multicheck":
            # Defensive: check widget existence before accessing text
            if self.text_title is None or self.text_widget is None:
                return {"type": subtype, "text": "", "items": []}

            title = self.text_title.text().strip()
            # Get list of non-empty trimmed lines from QTextEdit
            items = [
            ...
            # For other subtypes, return text from QLineEdit or empty string
            if self.text_widget is not None and isinstance(self.text_widget, QLineEdit):
                text = self.text_widget.text().strip()
            else:
                text = ""
            return {
                "type": subtype,
    ...
    def rebuild_entry(self):
        ...
        if subtype == "multicheck":
            # Build UI for multicheck type with title and multiline items
        ...
        # Validate inputs immediately after rebuilding the UI
        self.validate_input()
